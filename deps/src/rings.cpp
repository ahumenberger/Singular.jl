...
auto imap(ip_sring* src, ip_sring* dst, spolyrec* p){
    ring r = src;
    rChangeCurrRing(dst);
    ...
    // return
    return (poly)res->Data();
}

auto fetch(ip_sring* src, ip_sring* dst, spolyrec* p){
    ring r = src;
    rChangeCurrRing(dst);
...
void singular_define_rings(jlcxx::Module & Singular)
{
   ...
   Singular.method("p_Diff", [](poly p, int i, ring r) {
    ...
    Singular.method("imap", &imap);
    Singular.method("fetch", &fetch);
}
